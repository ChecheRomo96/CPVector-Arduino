	...
	//! @tparam T Data type to be used for the Dynamic Array
		...
		public:
			...
			//! @{
				...
				//! If an allocation fails and exceptions are enabled (CPVECTOR_EXCEPTIONS_ENABLED), then the method throws a CPVector::bad_allocation exception.\n
				...
				//! @tparam value The value to initialize the new elements with.
					...
					bool resize(uint32_t new_size, const T& value = T()){
						...
						{ 
							...
							catch(const CPVector::length_error& ex){
								throw;
							}
							catch(const CPVector::logic_error& ex){
								throw;
							}
							catch(const CPVector::exception& ex){
								throw;
							}
							catch(...){
						    	throw;
